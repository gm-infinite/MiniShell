...
/*   Updated: 2025/07/14 14:35:32 by emgenc           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/*
** ============================================================================
** SIGNAL HANDLING AND TERMINAL CONTROL SYSTEM
** ============================================================================
** 
** This file implements the sophisticated signal handling system for the minishell,
** providing proper Unix shell behavior for interactive terminal control and
** process management. Signal handling is critical for creating a responsive
** shell that behaves correctly under various user input conditions and system
** events.
**
** SIGNAL HANDLING PHILOSOPHY:
** ===========================
** The shell implements a dual-mode signal handling approach:
**
** 1. INTERACTIVE MODE (Parent Shell):
**    - SIGINT (Ctrl+C): Interrupts current operation, displays new prompt
**    - SIGQUIT (Ctrl+\): Ignored to prevent shell termination
**    - Signals integrate with GNU readline for proper terminal behavior
**
** 2. CHILD PROCESS MODE:
**    - SIGINT (Ctrl+C): Allows normal termination with status reporting
**    - SIGQUIT (Ctrl+\): Allows normal termination with quit message
**    - Restores default Unix signal semantics for executed commands
**
** READLINE INTEGRATION:
** ====================
** The signal handlers integrate deeply with GNU readline library:
** - rl_on_new_line(): Informs readline that cursor is on new line
** - rl_replace_line(): Clears current input buffer
** - rl_redisplay(): Refreshes prompt display
** This integration ensures clean prompt behavior after signal interruption.
**
** GLOBAL SIGNAL STATE:
** ===================
** The system uses a global variable (g_signal) to communicate signal
** events between signal handlers and the main shell loop. This approach
** is necessary because signal handlers have limited safe operations and
** cannot perform complex readline manipulation directly.
**
** SA_RESTART SEMANTICS:
** ====================
** The SA_RESTART flag is crucial for proper shell behavior:
** - Automatically restarts interrupted system calls
** - Prevents readline from being interrupted by signals
** - Ensures consistent user experience during input collection
** - Critical for proper integration with GNU readline library
**
** PROCESS SAFETY:
** ==============
** Signal handling in a multi-process shell requires careful coordination:
** - Parent shell maintains interactive signal behavior
** - Child processes restore standard Unix signal semantics
** - Signal mask manipulation prevents race conditions
** - Proper signal handler setup prevents undefined behavior
*/
...
/*
** ============================================================================
** INTERACTIVE MODE SIGNAL HANDLERS
** ============================================================================
*/
...
** SIGQUIT (Ctrl+\): Do nothing (ignore)
*/
...
** - SIGINT (Ctrl+C): Custom handler in parent, default in child
** - SIGQUIT (Ctrl+\): Ignore in parent, default in child  
** - SIGTERM: Handle gracefully
*/
